...
//		 : 객체의 초기 상태를 설정
		// new Book("Little Prince", "Saint"); -> 사용할 수 없음
						
		// 클래스명의 클래스의 새로운 객체를 생성, 해당 객체에 대한 참조(주소)를 myBook 변수에 저장
		// myBook은 'Book'이라는 '클래스 타입'을 가짐
		// 앞의 Book은 클래스타입 , 뒤의 Book은 생성자
		Book myBook = new Book("Little Prince", "Saint"); //생성자 호출
...
//		: 객체의 메서드(행동)를 호출하기 위해 점(.)연산자 사용
		// 메서드에 system.out.println 저장해놓은 출력문 그대로 사용했지만(함수 호출X)
		// 다른 행동의 경우 함수 호출하여 사용해야 할수 있다.
