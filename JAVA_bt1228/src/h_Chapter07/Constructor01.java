...
class Data_2 {
	...
	// : 매개변수 X, 아무런 내용 X
	// - 특별히 인스턴스 초기화가 요구되지 않으면 자바가 제공하는 기본 생성자 사용 > 메모리 사용줄임
	// Data_2() {}
...
class Car {
	...
	// 인스턴스 생성 후 값을 변경하기 위해 초기화 작업해야함
	Car() {} // 기본 생성자 - 차를 생성하고 추후 작업
	
	
	// this
	// 생성자 내에서 매개변수와 초기화하려는 인스턴스 변수의 이름이 동일한 경우
	// , 변수이름의 '구분'을 위해 this키워드를 사용, 객체 자신을 가리키는 참조 변수 this
	
	// 매개변수 사용하면 기본 생성자와 달리 인스턴스 생성함과 동시에 원하는 값으로 초기화됨
	...
	// : 생성자 간의 호출 사용
	// - 생성자의 이름으로 클래스 이름 대신 this를 사용 (CarX)
...
//		Car 클래스 사용
		...
		// 객체를 생성한 후에 인스턴스 값을 변경하기 위해 또 다시 초기화하지 않아도 됨 > 간결함
		Car c2 = new Car("white", "auto", 2);
		c2.displayInfo();
		
		// 위에서 매개변수 3개를 지정하는 생성자를 사용하여 객체의 틀을 정의하고
		// 색상만 변경하고 싶은 경우 사용
		Car c3 = new Car("blue");
		c3.displayInfo(); 
		...
		// : 모든 인스턴스 메서드에서 지역변수로 생략되어 존재 (this. 써서 생략)
