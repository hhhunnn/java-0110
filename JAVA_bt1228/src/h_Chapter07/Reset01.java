...
class Text {
	...
	// 인스턴스 변수 : 자동으로 초기화 가능(사용하기 전 초기화 안해도 됌)
	int x;
	int y = x; // x는 초기화하지 않아도 int형의 기본값인 0으로 초기화되어 있음
	
	void method() {
	// 지역 변수 : 자동으로 초기화 불가능(사용하기 전 초기화 해야함)
		int i; // 선언 가능 (사용하기 전)
//		int j = i; // i변수를 사용하기 전에 초기화 하지 않아 오류 - Error

		// x, i는 초기화 하지 않았고 y, j는 초기화를 하는데 초기화 하지 않은 x, i를 사용
		// y는 인스턴스 변수로 초기화하지 않은 x로 초기화 가능 
		// j는 지역 변수로 초기화 하지 않은 i로 초기화 불가능		
	}
}
